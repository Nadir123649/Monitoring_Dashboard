 const dataOne = [
      { name: 'Verification Success', value: 72, colors: ['#00d4ff', '#006eff', '#0047ff'] },
      { name: 'Abandoned by Customer', value: 16, colors: ['#ff006e', '#8338ec', '#3a86ff'] },
      { name: 'Verification Failure', value: 11, colors: ['#06ffa5', '#00d4aa', '#0056b3'] },
    ...
    Highcharts.chart('pieContainer', {
      ...
      title: {
        useHTML: true, text: `<div style="text-align:center;padding:25px">
      ...
      </div>`, align: 'center', verticalAlign: 'middle', floating: true, y: 0
      },
      credits: { enabled: false },
      plotOptions: {
        pie: {
          innerSize: '55%', depth: 45, borderRadius: 25, cursor: 'pointer',
          dataLabels: { enabled: true, format: '{point.percentage:.0f}%', style: { fontSize: '12px', fontWeight: '900', color: '#111' }, distance: -22 },
          animation: { duration: 2500 }
        }
      },
      tooltip: {
        backgroundColor: 'white', borderRadius: 20, borderWidth: 0,
        shadow: { color: 'rgba(0,212,255,0.3)', offsetX: 0, offsetY: 10, opacity: 0.3, width: 25 },
        style: { fontSize: '13px', fontWeight: '600', color: '#fff' },
        formatter() {
          return `<div style="padding:20px;text-align:center">
            <div style="font-weight:900;color:#111;margin-bottom:10px;font-size:15px;text-transform:uppercase">${this.point.name}</div>
      ...
      series: [{
        name: 'Data', data: dataOne.map((d, i) => ({
          name: d.name, y: d.value,
          color: { linearGradient: { x1: 0, y1: 0, x2: 1, y2: 1 }, stops: d.colors.map((c, j) => [j / (d.colors.length - 1), c]) },
          sliced: true
        })), animation: { duration: 2500 }
      }]
    });
    ...
    dataOne.forEach((d, i) => {
      ...
      box.className = 'flex items-center gap-2 px-4 py-3 transition transform cursor-pointer bg-white/95 backdrop-blur-lg rounded-xl hover:-translate-y-1 hover:scale-105';
      ...
      plotOptions: {
        bar: {
          ...
          dataLabels: {
            enabled: true,
            format: '{y:.1f}%',
            style: { fontSize: '12px', fontWeight: '500', color: '#1e293b', textOutline: 'none' },
            align: 'center',
            verticalAlign: 'top',
            y: -10
          }
